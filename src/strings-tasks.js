...
function isPalindrome(str) {
  ...
  return lowStr === reverseString(lowStr);
}
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    inverted += str[i] === upper ? str[i].toLowerCase() : upper;
  }
  return inverted;
}
...
function unbracketTag(str) {
  const a = removeFirstOccurrences(str, '<');
  const b = removeLastOccurrences(a, '>');
  return b;
}
